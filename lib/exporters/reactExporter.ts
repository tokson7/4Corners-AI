...
/**
 * Export React hook for using design system
 */
export function exportReactHook(designSystem: DesignSystem): string {
  ...
  code += `  return useMemo(() => designSystem, []);\n`;
  code += `}\n`;
...
/**
 * Export complete React package
 */
export function exportReactPackage(designSystem: DesignSystem): {
  constants: string;
  hook: string;
} {
  return {
    constants: exportReactConstants(designSystem),
    hook: exportReactHook(designSystem),
  };
}
